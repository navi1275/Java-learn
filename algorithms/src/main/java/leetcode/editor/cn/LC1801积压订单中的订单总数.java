 ...
 * 2021-05-20 13:32:38
 */
public class LC1801积压订单中的订单总数{
    public static void main(String[] args) {
//          Solution solution = new LC1801积压订单中的订单总数().new Solution();
...
class Solution {
        class Order{
            ...
            public Order(Integer num, Integer price, Integer type) {
                this.num = num;
                this.price = price;
                this.type = type;
            }
    ...
    public int getNumberOfBacklogOrders(int[][] orders) {
        ...
        PriorityQueue<Order> buy = new PriorityQueue<Order>((o1, o2) -> {
            return o2.price - o1.price;
        });

        PriorityQueue<Order> sell = new PriorityQueue<Order>((o1, o2) -> {
            return o1.price - o2.price;
        });

        for (int i = 0; i < orders.length; i++){
            int[] originalOrder = orders[i];
            Order order = new Order(originalOrder[1], originalOrder[0], originalOrder[2]);
            System.out.println("记录订单:" + order);
            if(originalOrder[2] == 0){
                while (!sell.isEmpty() && order.num > 0 && sell.peek().price <= order.price){
                    int min = Math.min(sell.peek().num, order.num);
                    order.num -= min;
                    sell.peek().num -= min;
                    if(sell.peek().num == 0){
                        sell.poll();
                ...
                if(order.num > 0){
                    System.out.println("记录采购:" + order);
                    buy.add(order);
                }

            }else{
                while (!buy.isEmpty() && order.num > 0 && buy.peek().price >= order.price){
                    int min = Math.min(buy.peek().num, order.num);
                    order.num -= min;
                    buy.peek().num -= min;
                    if(buy.peek().num == 0){
                        buy.poll();
        ...
        int mod = 1000000007;
        while (!sell.isEmpty()){
            Order poll = sell.poll();
            System.out.println("记录销售队列:" + poll);
            sum = (sum + poll.num) % mod;
        }
        while (!buy.isEmpty()){
            Order poll = buy.poll();
            System.out.println("记录采购队列:" + poll);
            sum = (sum + poll.num) % mod;
        }
        return sum;
    }
